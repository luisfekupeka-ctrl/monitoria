        ...
        <AnimatePresence mode="popLayout">
          {filteredUsers.map((user, index) => (
            <motion.div 
              ...
              className={cn(
                "bg-white p-6 rounded-[2rem] border transition-all group relative overflow-hidden",
                user.approved ? "border-slate-200" : "border-amber-200 bg-amber-50/30"
              )}
            >
              {!user.approved && (
                <div className="absolute top-0 right-0 px-4 py-1.5 bg-amber-100 text-amber-700 text-[10px] font-black uppercase tracking-widest rounded-bl-2xl">
              ...
              <div className="flex items-start justify-between mb-6">
                <div className={cn(
                  ...
                  user.role === 'admin' ? "bg-sesi-blue text-white shadow-sesi-blue/20" : "bg-white border border-slate-100 text-slate-400 shadow-sm"
                )}>
                ...
                >
                  {user.approved ? <XCircle size={16} /> : <CheckCircle2 size={16} />}
